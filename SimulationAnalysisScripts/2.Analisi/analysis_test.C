void histograms(ROOT::RDF::RInterface<ROOT::Detail::RDF::RLoopManager, void> Events, std::vector<unsigned int> SurfaceIDs, int nBin, double Emin, double Emax, string option){
    ...
    TFile *file = new TFile("Graphs/Graphs_test.root", option.c_str());
    ...
    vector<ROOT::RDF::RResultPtr<::TH1D>> Histos;

    for(int i = 0; i < SurfaceIDs.size(); i++){
        string filter = "SurfaceID == " + std::to_string(SurfaceIDs.at(i));
        // cout<<filter<<endl;
        Histos.push_back(Events.Filter(filter).Histo1D<double, double>({Form("%s_Energy_Det%d", particella.c_str(), SurfaceIDs.at(i)), Form("%s energy Det %d; E (GeV); Particles/EOT", particella.c_str(), SurfaceIDs.at(i)), nBin, Emin, Emax}, "Ekin", "Peso"));
    }

    ROOT::RDF::RunGraphs({Histos.begin(), Histos.end()});

    int i = 0; 
    for (auto energy : Histos){
        string filter = "SurfaceID == " + std::to_string(SurfaceIDs.at(i));
        energy->Write();
...
void analysis_test(double NormFactor = 1, string files = "root_files/*.root", bool debug = true){
